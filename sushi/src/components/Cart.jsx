...
const Cart = () => {
    ...
    const { listCart, subTotal, reset, remove, plusCount, minusCount } = useContext(CartContext);
    ...
    return (<>
        ...
        <section id={StyleCart.cart}>
            ...
            {listCart.map((list) => (
                                ...
                                <section className={StyleCart.products_cart}>
                                    ...
                                    <picture className={StyleCart.picture_product}>
                                        <img src={list.image} alt="Foto miniatura del producto a la venta"/>
                                    </picture>
                                    <p className={StyleCart.precio_product}><span>$</span>{list.costo}</p>
            
                                    <button className={StyleCart.btn_trash_product_cart} onClick={(evento
                                        ) => {
                                    ...
                                    <article className={StyleCart.btn_product_cart}>
                                        ...
                                        <button className={StyleCart.btn_plus_cart}><i className="fa-solid fa-plus" onClick={(evento) => {
                                            evento.preventDefault();
                                            countPlus(list.name, list.contador);
                                        }}></i></button>
                                        <span className={StyleCart.cantidad_product_cart}>{list.contador}</span>
                                        <button className={StyleCart.btn_minus_cart}><i className="fa-solid fa-minus" onClick={(evento) => {
                                            evento.preventDefault();
            ...
            <section id={StyleCart.section_subtotal_cart}>
                ...
                <p id={StyleCart.text_pago_cart}><span>$</span>{subTotal}</p>
            </section>
            <section id={StyleCart.section_btn_cart}>
                <button id={StyleCart.btn_trash_cart} onClick={(evento) => {
                    evento.preventDefault();
                    resetCart();
                }}
                ><i className="fa-solid fa-trash"></i></button>
